...
class vehEnv:
    ...
    def step(self, action, throttle,P,I,D): 
        ...
        if (a != 0.0):
            ...
            self.beta0 = np.arctan((self.param_mpc[0]*np.tan(0.0)+self.param_mpc[1]*np.tan(self.useq[0]))/(self.param_mpc[0]+self.param_mpc[1]))
        ...
        print(self.xseq)
        self.data = np.append(self.data,[[self.Xnew,self.Ynew,self.vxfilt,self.vx,throttle,self.x0d[0],self.x0d[1],self.Angnew,self.n,self.distoff,self.u,self.timediff,action,P,I,D]],axis = 0)
