...
class vehEnv:
    def __init__(self, hedge, T=10,rho=0):
        ...
        # self.lb = dict(x=np.array([-np.inf, -np.inf, -np.inf]), u=np.array([-0.30]), Du=np.array([-0.10]))
        # self.ub = dict(x=np.array([np.inf, np.inf, np.inf]), u=np.array([0.30]), Du=np.array([0.10]))
        self.lb = dict(x=np.array([-np.inf, -np.inf, -np.inf]), u=np.array([-0.2]), Du=np.array([-0.04]))
        self.ub = dict(x=np.array([np.inf, np.inf, np.inf]), u=np.array([0.2]), Du=np.array([0.04]))
        self.udiscrete = np.array([False])
    ...
    def stagecost(self, x, u, xsp, du):
        ...
        return 3.0*(offset) + 20.32*(du**2) + 11.06*(u**2)
        # return 3.0*(offset) + 5.6*(du**2) + 2.2*(u**2)
        # return 2.0*(x[1] - 4*np.sin(2*pi/50*x[0]))**2 + 3.0*u[0]**2 

    def event_cost(self):
        if (np.abs(self.distoff) >= 0.035):
            action = 1
        else:
            action = 0
        return action
    ...
    def step(self, action, throttle,P,I,D): 
        ...
        self.beta0 = np.arctan((self.param_mpc[5]*np.tan(0.0)+self.param_mpc[6]*np.tan(self.useq[0]))/(self.param_mpc[5]+self.param_mpc[6]))
        ...
        #     done = False
        print(self.xseq)
        self.data = np.append(self.data,[[self.Xnew,self.Ynew,self.vxfilt,self.vx,throttle,self.x0d[0],self.x0d[1],self.Angnewfilt,self.n,self.distoff,self.u,self.timediff,action,P,I,D]],axis = 0)
...
while True:
    try:
        ...
        # steeringmap = -trim(steering[0]*2.65) # was 3.34 for linear approx to +/-1
        steeringmap = trim((-176.36*(steering[0]**5))-(34.572*(steering[0]**4))+(14.168*(steering[0]**3))+(1.9324*(steering[0]**2))-(2.9107*steering[0]))
        # steeringmap = -0.1
        steeringAngint = int((steeringmap+1.0) * 100.0)
        steerbytesf = steeringAngint.to_bytes(2,'little')
        velerror = 1.6 - vxfiltered
        # velerror = 1.0 - vxfiltered
        throttle, Isum, accelprev, P, I, D = velPID(velerror,velerrorprev,accelprev,Isum,timediff,steeringmap)
        velerrorprev = velerror
        throttle = 23000
        # throttle = 7000
        throttleint = int(throttle) # * -1000000000000.0)
        throttlebytes = throttleint.to_bytes(6,'little')
        steerbytes = b''.join([steerbytesf,throttlebytes])
        # steerbytes = steerbytesf + throttlebytes
        # timetest1 = time.time()
        if action == 0:
            time.sleep(np.clip(0.20-(time.time()-timebegin),0,None))
        # s.sendto(steerbytes,('192.168.8.126', 9999))
        s.send(steerbytes)
        # print(int.from_bytes(s.recvfrom(1)[0],'little'))
        # timetest2 = time.time() - timetest1
        # print(f"comm time is {timetest2}")

        # time.sleep(0.03)
    except:
        hedge.stop()  # stop and close serial port
        np.savetxt('mpckinemdata_1_3_07_1dt_2ws_035ET_small', data, delimiter = ' , ')
        sys.exit()