...
class vehEnv:
    def __init__(self, hedge, T=10,rho=0):
        ...
        self.l = mpc.getCasadiFunc(self.stagecost, [self.Nx, self.Nu, self.Nx, self.Nu, 2], ["x", "u", "x_sp", "Du", "gainlist"])
        ...
        self.gainrow = rho
        self.gains = np.loadtxt('2DOF_DOEarrayinsidelarge_2_18', delimiter=' , ')
        self.gains = np.vstack(([15.6,13.2],self.gains))
        self.gains = np.vstack((self.gains,[15.6,13.2]))
    ...
    def veh_rhs(self, x0, t, u0, vx = 0.65):
    ...
    def stagecost(self, x, u, xsp, du, gainlist):
        ...
        return 3.0*(offset) + gainlist[0]*(du**2) + gainlist[1]*(u**2)
    ...
    def step(self, action, throttle,P,I,D): 
        ...
        #     done = False
        print(self.xseq)
        self.data = np.append(self.data,[[self.Xnew,self.Ynew,self.vxfilt,self.vx,throttle,self.x0d[0],self.x0d[1],self.Angnewfilt,self.n,self.distoff,self.u,self.timediff,P,I,D]],axis = 0)
...
def steerloop(Isum,velerrorprev,accelprev,throttle,P,I,D):
    ...
    # steerbytesr = steeringAngintr.to_bytes(1,'little')
    ...
    steerbytes = b''.join([steerbytesf,throttlebytes])#steerbytesf + throttlebytes
...
for i in range(18):
    np.savetxt('mpckinemdata_1_2_20_1dt_2ws_caltest_gain{}'.format(i), datalist[i], delimiter = ' , ')