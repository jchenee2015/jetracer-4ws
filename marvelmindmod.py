                            ...
                            try:
                                if (len(bufferList) > pktHdrOffset + 4 + msgLen + 2):
                                    ...
                                    if CRC_calc == usnCRC16:
                                        if (isMmMessageDetected or isCmMessageDetected): # modified to provide velocity information instead of z position
                                            self.positionUpdated= True
                                            value = [usnAdr, usnX, usnY, usnZ, usnAngle, usnTimestamp]
                                            valueold = list(self.valuesUltrasoundPosition)[-1]
                                            vel = np.sqrt((usnX-valueold[1])**2+(usnY-valueold[2])**2)/((usnTimestamp-valueold[5])*0.001)
                                            value[3] = vel
