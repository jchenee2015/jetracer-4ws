...
class vehEnv:
    ...
    def step(self, action, throttle,P,I,D): 
        ...
        #     done = False
        print(self.xseq)
        self.data = np.append(self.data,[[self.Xnew,self.Ynew,self.vxfilt,self.vx,throttle,self.x0d[0],self.x0d[1],self.Angnewfilt,self.n,self.distoff,self.u[0],self.u[1],self.timediff,action,P,I,D]],axis = 0)
...
while True:
    try:
        ...
        # steerbytes = steerbytesf + steerbytesr + throttlebytes
        # timetest1 = time.time()
        if action == 0:
